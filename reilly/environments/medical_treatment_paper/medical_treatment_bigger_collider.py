...
Rewards:
    ...
    - 1: healthy 
'''
class MedicalTreatmentBiggerCollider(MedicalTreatment):
    ...
    def _run_step(self, action):
        ...
        self._state[0] = np.random.binomial(size=1, n=1, p= self._next_state_probs[str([self._state[0], action])])[0] # P(S=1)

        # Compute M = self._state[1]
        self._state[1] = np.random.binomial(size=1, n=1, p= self._next_M_probs[str([self._state[1], action, reward])])[0] # P(M=1)

        # Compute E = self._state[2]
        self._state[2] = np.random.binomial(size=1, n=1, p= self._next_E_probs[str([self._state[2], action, reward])])[0] # P(E=1)
